...
## 真机验证（2026-09-04）——**四个缺陷全修完后，三项全过**

### 第二轮（缺陷全修完、三个 PR 合入 master 之后）：三项全过

| # | 检查点 | 结果 | 证据 |
|---|---|---|---|
| 1 | 装一次 | ✅ | 取脚本成功 → pin 解析出 `152.0.7977.64` → 中文正常「==> 下载 Chrome for Testing 152.0.7977.64（win64）」→ 落到 `build/bin/chromium/chrome.exe` → 界面转 `installed`、显示路径、按钮变「重新安装」 |
| 1b | **脚本还在跑的时候就收到输出行** | ✅ | 这是 C-1 修复的核心，也是本分支的 Goal。旧实现要等脚本退出才一次性倒出 |
| 2 | **装到一半关掉设置面板再打开** | ✅ | 关掉 20 秒再开：`status` 仍是 `installing`、日志还在并重新渲染。安装没被打断——**这是整个设计要保的性质，也是唯一自动化测不到的那条** |
| 3 | 重装确认框 | ✅ | 弹出「重新安装会先删除现在这个，再下载新的。下载中途失败，这台机器上就既没有旧的、也没有新的」；点取消后 `status` 与日志**一字未动**，没有触发任何下载 |

验证后已清掉真机装出来的 `build/bin/chromium`。

**环境坑（新增）**：`wails dev` 触发了 Go 编译器 ICE（`cmd/compile` 崩在 `compileFunctions`），
加 `GOFLAGS=-p=2` 才起得来——与本仓 §七 记的「`go test ./...` 默认并发会崩工具链」同源。

### 第一轮：三项一项没跑完，因为撞出四个缺陷

原定三项，**第一项就没过**。真机跑出来的四个缺陷，全部是自动化测试测不到的那类：
