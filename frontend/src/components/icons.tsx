...
// Icon is the shared wrapper for the inline Lucide-style SVG icon set. All icons
// share a 24x24 viewBox, inherit the current text color via stroke=currentColor,
// and default to a 16px (w-4 h-4) footprint. Replacing the app's emoji/glyph
// icons with this set gives every control a consistent, professional look and a
// stable size (no font-dependent rendering).
function Icon({ className = 'w-4 h-4', ...props }: SVGProps<SVGSVGElement>) {
  return (
    <svg
...
export function SpinnerIcon({ className = 'w-4 h-4', ...props }: SVGProps<SVGSVGElement>) {
  return (
    <Icon className={`${className} animate-spin`} {...props}>
...
// BotIcon represents the agent's identity (used in the empty chat state).
...
// FileIcon marks a plain file row in the workspace file tree (as opposed to
